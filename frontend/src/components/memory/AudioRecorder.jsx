  ...
  if (existingUrl && !audioUrl) {
    return (
      <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
        <label style={{ fontSize: '12px', color: '#9CA3AF', fontFamily: "'DM Sans', sans-serif" }}>🎙 Voice Note</label>
        <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
          <audio src={existingUrl} controls style={{ height: '32px', flex: 1 }} />
          {onClear && (
            <button type="button" onClick={onClear} style={{ background: 'none', border: '1px solid #4B5563', color: '#6B7280', borderRadius: '6px', padding: '6px 10px', cursor: 'pointer', fontSize: '12px' }}>✕</button>
          )}
        </div>
      </div>
    )
